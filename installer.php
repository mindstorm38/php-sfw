...
$version_path = file_build_path( PHP_HELPER_DIR, VERSION_FILE );

$update = file_exists( $version_path );
$current_version_commit_sha = $update ? explode( "\n", file_get_contents( $version_path ) )[0] : "master";
...
$tmp_folder_path = file_build_path( PHP_HELPER_DIR, TMP_FOLDER );
$tmp_download_file = file_build_path ( $tmp_folder_path, "{$latest_commit_sha}.zip" );
$src_folder_path = file_build_path( PHP_HELPER_DIR, SRC_FOLDER );

// Clearing existing downloaded file
if ( !is_dir( PHP_HELPER_DIR ) ) mkdir( PHP_HELPER_DIR );
if ( !is_dir( $tmp_folder_path ) ) mkdir( $tmp_folder_path );
...
if ( is_dir( $src_folder_path ) ) rmdir_recursive( $src_folder_path );
...
for ( $i = 0; $i < $zip_archive->numFiles; $i++ ) {
	...
	if ( preg_match( "/^src\/(.*)/", $name, $matches ) === 1 ) {
		...
		copy( "zip://{$tmp_download_file}#{$raw_name}", file_build_path( $src_folder_path, $matches[ 1 ] ) );
...
file_put_contents( $version_path, $latest_commit_sha );

println( "Removing tmp folder..." );
rmdir_recursive( $tmp_folder_path );
